...
export class HotelService {
                ...
                this.books = this.books.map((book) => {
                    if (book.id === bookingId) {
                        if (book.status == 'reserved') {
                            Swal.fire(`Tamu ${book.reservee.name} sudah check-in pada kamar ${book.roomNumber}!`);
                            book.status = 'checked-in';
                        } else {
                            Swal.fire({
                                icon: 'error',
                                title: 'Eh...',
                                text: `Statusnya udah ${book.status}, ngga bisa dijadiin checked-in!`,
                ...
                this.books = this.books.map((book) => {
                    if (book.id === bookingId) {
                        if (book.status == 'checked-in') {
                            Swal.fire(`Tamu ${book.reservee.name} sudah check-out pada kamar ${book.roomNumber}!`);
                            book.status = 'checked-out';
                        } else if(book.status == 'reserved') {
                            Swal.fire({
                                icon: 'error',
                                title: 'Eh...',
                                text: `Statusnya masih ${book.status}, ngga bisa di-checked-out!`,
                                footer: '<a href="">Kenapa ini?</a>',
                            });
                        } else {
                            Swal.fire({
                                icon: 'error',
                                title: 'Eh...',
                                text: `Statusnya udah ${book.status}, ngga bisa checked-out lagi!`,
                                footer: '<a href="">Kenapa ini?</a>',
                            });
                        }
                    }
    ...
    remove(bookingId: number): Observable<void> {
        return new Observable<void>((observer: Observer<void>) => {
            try {
                const bookId: number = this.books.findIndex(item => item.id == bookingId);
            ...
            try {
                if(book.status === 'checked-out'){
                    Swal.fire({
                        icon: 'error',
                        title: 'Error!',
                       text: `Data tamu ${book.reservee.name} tidak dapat diubah karena status pemesanan sudah ${book.status}`,
                        footer: '<a href="">Kenapa ini?</a>',
                    });
                } else {
