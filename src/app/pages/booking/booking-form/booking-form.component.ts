...
import { IBookFormComponent } from '../interface/guest-book.interface';
import { BookField } from '../model/book-field.model';
import { Book, Guest } from '../model/guest-book.model';
...
export class BookingFormComponent implements OnInit, IBookFormComponent {
  ...
  bookingGroup: FormGroup = new FormGroup({
    ...
    [BookField.ROOM_NUMBER]: new FormControl(null,
      [Validators.required]
    ),
    [BookField.DURATION]: new FormControl(null,
      [Validators.required]
    ),
    [BookField.GUEST_COUNT]: new FormControl(null,
      [Validators.required]
    )
  })
  ...
  onSubmitReservation(): void {
    ...
    if(this.id){
      ...
      Swal.fire(`Tamu ${book.reservee.name} telah melakukan pemesanan untuk kamar ${book.roomNumber} selama ${book.duration} malam dengan total harga ${book.duration * this.fee}`)
    }
  ...
  setFormValue(): void {
    if(this.book){
      ...
      this.bookingGroup.get(BookField.EMAIL)?.setValue(this.book.reservee.email);
      this.bookingGroup.get(BookField.PHONE)?.setValue(this.book.reservee.phone);
      this.bookingGroup.get(BookField.ROOM_NUMBER)?.setValue(this.book.roomNumber);
      this.bookingGroup.get(BookField.DURATION)?.setValue(this.book.duration);
      this.bookingGroup.get(BookField.GUEST_COUNT)?.setValue(this.book.guestCount);
    } else {
      this.bookingGroup.reset();
    }
  }
